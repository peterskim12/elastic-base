...
wget https://artifacts.elastic.co/downloads/beats/metricbeat/metricbeat-5.0.2-linux-x86_64.tar.gz
wget https://artifacts.elastic.co/downloads/beats/packetbeat/packetbeat-5.0.2-linux-x86_64.tar.gz
...
cat <<KIBANA_CONF >> /opt/elastic/kibana-5.0.2-linux-x86_64/config/kibana.yml
server.host: "0.0.0.0"
KIBANA_CONF

# Recommended ES settings to pass bootstrap checks
# START BOOTSTRAP CHECKS CONFIG CHANGES #
cat <<ES_CONF >> /opt/elastic/elasticsearch-5.0.2/config/elasticsearch.yml
http.host: [_local_, _eth1_]
...
sed -i -e 's/Xms2g/Xms512m/g' /opt/elastic/elasticsearch-5.0.2/config/jvm.options
sed -i -e 's/Xmx2g/Xmx512m/g' /opt/elastic/elasticsearch-5.0.2/config/jvm.options
...
cd /opt/elastic/elasticsearch-5.0.2
...
cd /opt/elastic/kibana-5.0.2-linux-x86_64
