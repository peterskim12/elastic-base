...
cat <<KIBANA_CONF >> /opt/elastic/kibana-4.6.3-linux-x86_64/config/kibana.yml
reporting.encryptionKey: "FeelTheBern"
KIBANA_CONF

# Recommended ES settings
# START BOOTSTRAP CHECKS CONFIG CHANGES #
cat <<ES_CONF >> /opt/elastic/elasticsearch-2.4.3/config/elasticsearch.yml
...
cd /opt/elastic/elasticsearch-2.4.3
...
cd /opt/elastic/kibana-4.6.3-linux-x86_64
...
sudo -u vagrant bash -c 'bin/kibana plugin --install elastic/sense'
