...
wget https://artifacts.elastic.co/downloads/beats/metricbeat/metricbeat-5.1.1-linux-x86_64.tar.gz
wget https://artifacts.elastic.co/downloads/beats/packetbeat/packetbeat-5.1.1-linux-x86_64.tar.gz
...
cat <<KIBANA_CONF >> /opt/elastic/kibana-5.1.1-linux-x86_64/config/kibana.yml
server.host: "0.0.0.0"
KIBANA_CONF

# Recommended ES settings to pass bootstrap checks
# START BOOTSTRAP CHECKS CONFIG CHANGES #
cat <<ES_CONF >> /opt/elastic/elasticsearch-5.1.1/config/elasticsearch.yml
...
sed -i -e 's/Xms2g/Xms512m/g' /opt/elastic/elasticsearch-5.1.1/config/jvm.options
sed -i -e 's/Xmx2g/Xmx512m/g' /opt/elastic/elasticsearch-5.1.1/config/jvm.options
