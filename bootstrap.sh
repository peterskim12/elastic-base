...
cat <<KIBANA_CONF >> /opt/elastic/kibana-6.0.0-linux-x86_64/config/kibana.yml
server.host: "0.0.0.0"
KIBANA_CONF

# Recommended ES settings to pass bootstrap checks
# START BOOTSTRAP CHECKS CONFIG CHANGES #
cat <<ES_CONF >> /opt/elastic/elasticsearch-6.0.0/config/elasticsearch.yml
http.host: [_local_, _eth0_]
...
cat <<LS_CONF >> /opt/elastic/logstash-6.0.0/config/logstash.yml
...
cd /opt/elastic/elasticsearch-6.0.0
...
cd /opt/elastic/kibana-6.0.0-linux-x86_64
